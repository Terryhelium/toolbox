...
if ! command -v filebeat &> /dev/null; then
  ...
  echo "安装命令: sudo rpm -ivh filebeat-*.rpm"
  exit 1
else
  echo "✅ Filebeat 已安装"
  filebeat version
...
index: "filebeat-centos-%{+yyyy.MM.dd}"
...
if command -v getenforce &> /dev/null && [[ $(getenforce) != "Disabled" ]]; then
  ...
  semanage port -a -t http_port_t -p tcp 9200 2>/dev/null || true
  semanage port -a -t http_port_t -p tcp 5601 2>/dev/null || true
fi
